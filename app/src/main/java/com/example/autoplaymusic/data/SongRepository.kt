package com.example.autoplaymusic.data
import android.content.Context
import com.example.autoplaymusic.core.harmonica.RustMidiCompiler
...
import com.example.autoplaymusic.update.SyncedScore
...
data class Song(
    ...
    val rawMidiPath: String? = null,
    val midiTrack: Int? = null,
    val compilerVersion: Int = 0,
    val midiVoice: MidiVoice? = null,
) {
...
class SongRepository(
    ...
    private fun loadBuiltIns(): List<Song> {
        ...
        val rawBuiltIns =
            listOf(
                "rain-love.mid" to "雨爱",
                "spring-shadow.mid" to "春日影（Cover CRYCHIC）",
                "night-sky.mid" to "夜空中最亮的星",
                "phantom-listening.mid" to "幻听",
            ).mapNotNull { (asset, title) ->
                runCatching {
                    val bytes = appContext.assets.open(asset).use { it.readBytes() }
                    val rust =
                        RustMidiCompiler.compile(bytes, melody = true)
                            ?: error("Rust MIDI core 编译失败")
                    val score = rust.score
                    val bpm = rust.bpm
                    Song(asset.substringBeforeLast('.'), title, score, bpm, "MIDI · Rust 编译", true)
                }.getOrNull()
            }
            ...
            (0 until saved.length()).mapNotNull { index ->
                runCatching {
                    ...
                    Song(
                        ...
                        rawMidiPath = s.optString("rawMidiPath").ifBlank { null },
                        midiTrack = if (s.has("midiTrack") && !s.isNull("midiTrack")) s.optInt("midiTrack") else null,
                        compilerVersion = s.optInt("compilerVersion", 0),
                        midiVoice =
                            s
                                .optString(
                                    "midiVoice",
                                ).takeIf { it.isNotBlank() }
                                ?.let { runCatching { MidiVoice.valueOf(it) }.getOrNull() },
                    )
                }.getOrNull()
            }
        val refreshed = loaded.map(::recompileMidi)
        if (refreshed != loaded) write(refreshed)
        refreshed.forEach { merged[it.id] = it }
        return merged.values.toList()
    }

    private fun recompileMidi(song: Song): Song {
        val fileName = song.rawMidiPath ?: return song
        if (song.compilerVersion >= RustMidiCompiler.VERSION || !fileName.matches(Regex("[0-9a-fA-F-]+\\.mid"))) return song
        return runCatching {
            val file = java.io.File(java.io.File(appContext.filesDir, "midi_sources"), fileName)
            val rust =
                RustMidiCompiler.compile(file.readBytes(), song.midiTrack, song.midiVoice != MidiVoice.ONSETS)
                    ?: error("Rust MIDI core 编译失败")
            song.copy(score = rust.score, bpm = rust.bpm, compilerVersion = RustMidiCompiler.VERSION)
        }.getOrDefault(song)
    }
    ...
    fun addMidi(
        ...
        val dir = java.io.File(appContext.filesDir, "midi_sources").apply { mkdirs() }
        val file = java.io.File(dir, "$id.mid").apply { writeBytes(bytes) }
        val rust =
            RustMidiCompiler.compile(bytes, track, voice == MidiVoice.MELODY)
                ?: error("Rust MIDI core 编译失败")
        val score = rust.score
        val bpm = rust.bpm
        val song =
            Song(
                ...
                bpm,
                "MIDI · 引擎编译",
                rawMidiPath = file.name,
                midiTrack = track,
                compilerVersion = RustMidiCompiler.VERSION,
                midiVoice = voice,
            )
        write(songs().filterNot { it.builtIn } + song)
        selectedId = song.id
            ...
            json.put(
                JSONObject()
                    .put(
                        ...
                        "rawMidiPath",
                        s.rawMidiPath,
                    ).put("midiTrack", s.midiTrack)
                    .put("compilerVersion", s.compilerVersion)
                    .put("midiVoice", s.midiVoice?.name),
            )
        }
