...
import com.example.autoplaymusic.data.SyncApplyResult
...
class UpdateManager(
    ...
    fun checkApp(onResult: (Result<AppUpdate?>) -> Unit) {
        background(onResult) {
            val json = JSONObject(get("$baseUrl/api/update/stable"))
            val code = json.optInt("latestVersionCode", 0)
            if (code <= BuildConfig.VERSION_CODE) {
                null
            } else {
                AppUpdate(
                    code,
                    json.optString("latestVersionName", code.toString()),
                    resolve(json.getString("apkUrl")),
                    json.optString("apkSha256"),
                    json.optJSONArray("releaseNotes")?.let { array -> List(array.length()) { array.optString(it) } } ?: emptyList(),
                )
            }
        }
    }
    ...
    fun downloadPublicScore(
        ...
        background(onResult) {
            ...
            if (bytes.looksLikeLicense()) {
                val license = JSONObject(bytes.toString(Charsets.UTF_8))
                val midi = decryptLicensedMidi(license)
                RustMidiCompiler.compile(midi, melody = true)?.score ?: error("MIDI 解析失败")
            } else if (bytes.startsWithMidi()) {
                RustMidiCompiler.compile(bytes, melody = true)?.score ?: error("MIDI 解析失败")
            } else {
                bytes
    ...
    fun install(
        ...
        if (Build.VERSION.SDK_INT >= 26 && !activity.packageManager.canRequestPackageInstalls()) {
            ...
            onStatus("请允许安装未知应用后重试")
            return
        }
        val manager = activity.getSystemService(DownloadManager::class.java)
        val request =
            DownloadManager
                .Request(Uri.parse(update.apkUrl))
                .setTitle("落弦律 ${update.versionName}")
